			...
			switch (msg.what) {
				...
				case MSG_REQUEST: // 收到请求
					...
					if (msg.obj != null && msg.obj instanceof ImageRef) {
						...
						if (url.toLowerCase().contains("storage")||url.toLowerCase().contains("sdcard/")||url.toLowerCase().contains("mnt/")||url.toLowerCase().contains(Environment.getExternalStorageDirectory().toString())) {
	...
	private void setImageBitmap(ImageView imageView, Bitmap bitmap,
		...
		if (isTran) {
			final TransitionDrawable td = new TransitionDrawable(
					new Drawable[] {new ColorDrawable(Color.rgb(0, 0, 0)),new BitmapDrawable(bitmap) });
