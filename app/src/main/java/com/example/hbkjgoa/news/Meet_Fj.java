...
public class Meet_Fj extends Activity {
	private ListView listView;
	TextView Text2;
	private Meet_Bean ap;
	String[] names = null;
	...
	/*	for (int i = 0; i<aa.length; i++){
			...
			for (int i = 0; i<aa.length; i++){
				psid2 += "," + aa[i];
				names = psid2.substring(1, psid2.length()).split(",");
				FilePath = psid2.substring(1, psid2.length()).split(",");
				listView.setAdapter(new MyAdapter());
			}

		/*psid2 += "," + ap.getSLZZFJ_FullName();
		names = psid2.substring(1, psid2.length()).split(",");*/
		}else {
	...
	private void findview(){
		...
		ap = (Meet_Bean) intent.getSerializableExtra("fj");
		Text2.setText("附件");
	}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			vh.tv.setText(names[position]);
				...
				if( FilePath[position].contains(".JPG")|| FilePath[position].contains(".jpg")|| FilePath[position].contains(".png")|| FilePath[position].contains(".bmp")){
					...
					vh.im1.setImageResource(R.mipmap.xlsx);
			...
			convertView.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(FilePath[w].contains(".JPG")||FilePath[w].contains(".jpg")||FilePath[w].contains(".png")||FilePath[w].contains(".PNG")||FilePath[w].contains(".bmp")){
						...
						progressDialog= ProgressDialog.show(Meet_Fj.this,"正在获取数据中","请稍等...");
						new Thread(){
							@Override
							public void run() {
								...
								try {
									...
									file01=getFileFromServer(Meet_Fj.this.getResources().getString(R.string.http_url)+"/uploadfile/"+url,filename);
								} catch (Exception e) {
									e.printStackTrace();
								}
						...
						/*new AlertDialog.Builder(Meet_Fj.this).setMessage("是否下载查看？")
	...
	private void imageBrower(int position, String[] urls) {
		Intent intent = new Intent(Meet_Fj.this, ImagePagerActivity.class);
		// 图片url,为了演示这里使用常量，一般从数据库中或网络中获取
		intent.putExtra(ImagePagerActivity.EXTRA_IMAGE_URLS, urls);
		intent.putExtra(ImagePagerActivity.EXTRA_IMAGE_INDEX, position);
		Meet_Fj.this.startActivity(intent);
	}
	/**
	 * 说明：
	...
	private void initStatusBar() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			...
			StatusBarUtils.setStatusBarColor(Meet_Fj.this, R.color.btground);
		}
	}
}