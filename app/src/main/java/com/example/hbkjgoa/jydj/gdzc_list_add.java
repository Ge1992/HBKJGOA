...
public class gdzc_list_add extends Activity implements OnItemClickListener, XListView.IXListViewListener {
	private Dialog progressDialog;
	private String json,searchstr="",type="",json2="";
	private boolean webbing = false;
	private List<GDZC_Bean> list = new ArrayList<GDZC_Bean>();
	private XListView mListView;
	private GDZCAdapter_add adapter;
	...
	public static gdzc_list_add listact;
	...
	private void setData(List<GDZC_Bean> listitem) {
		adapter = new GDZCAdapter_add(gdzc_list_add.this, listitem);
		mListView.setAdapter(adapter);
	}
	...
	private void findview(){
		...
		serch.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				final EditText inputServer = new EditText(gdzc_list_add.this);

				new AlertDialog.Builder(gdzc_list_add.this,AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("输入责任人姓名")//使用默认设备 浅色主题
						.setView(inputServer)
	...
	private void getInfo() {
		boolean havenet = NetHelper.IsHaveInternet(gdzc_list_add.this);
		if (havenet) {
			LoadingDialog.Builder builder1=new LoadingDialog.Builder(gdzc_list_add.this)
					.setMessage("加载中...")
					.setCancelable(false);
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					json = WebServiceUtil.everycanforStr("search", "userid", "type", "pageNo",searchstr, spname, type, 1, "GetJYSheBeiList");

					Log.d("gsp", "GetJYSheBeiList" + ":" + json);
			...
			ToastUtils.showToast(gdzc_list_add.this, "网络连接失败！");
		}
	}
	...
	public void onRefresh() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				webbing = true;
				json = WebServiceUtil.everycanforStr("search", "userid", "type", "pageNo",searchstr, spname, type, pageno, "GetJYSheBeiList");
	...
	/*	new SJ_Bean();
		...
		intent.setClass(gdzc_list_add.this, InfoGDZC.class);
	...
	private  void gettype(){
		new Thread() {
			@Override
			public void run() {
				json2 = WebServiceUtil.everycanforStr("", "", "", "","", "", "", 0, "GetJYSheBeiType");
	...
	private void initStatusBar() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			...
			StatusBarUtils.setStatusBarColor(gdzc_list_add.this, R.color.btground);
		}
	}
}
