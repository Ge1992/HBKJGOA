...
public class gdzc_list3 extends Activity implements OnItemClickListener, XListView.IXListViewListener {
	...
	private String lx="",name="",id="",uname="",xh="",spname,method="GetSheBeiList",tile="";
	public static gdzc_list3 listact;
	private String[] bb;
	TabLayout tabLayout;
	...
	private void setData(List<GDZC_Bean> listitem) {
		adapter = new GDZCAdapter(gdzc_list3.this, listitem);
		mListView.setAdapter(adapter);
	}

	private void findview(){
		method = this.getIntent().getStringExtra("method");
		...
		serch.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				final EditText inputServer = new EditText(gdzc_list3.this);

				new AlertDialog.Builder(gdzc_list3.this,AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("输入设备名称")//使用默认设备 浅色主题
						.setView(inputServer)
	...
	private void getInfo() {
		boolean havenet = NetHelper.IsHaveInternet(gdzc_list3.this);
		if (havenet) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					json = WebServiceUtil.everycanforStr("search", "userid", "type", "pageNo",searchstr, spname, type, 1, method);
			...
			ToastUtils.showToast(gdzc_list3.this, "网络连接失败！");
		}
	}
	...
	public void onRefresh() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				webbing = true;
				json = WebServiceUtil.everycanforStr("search", "userid", "type", "pageNo",searchstr, spname, type, pageno, method);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		name=localGZRZ.getSheBeiName();
		id=localGZRZ.getID();
		uname=localGZRZ.getGuanLiUser();
		xh=localGZRZ.getXingHao();
	...
	private void initStatusBar() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			...
			StatusBarUtils.setStatusBarColor(gdzc_list3.this, R.color.btground);
		}
	}
}
